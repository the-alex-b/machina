...
class ClickablePlot(QMainWindow):
    ...
    def clear_plot(self):
        ...
        self.collection.code = self.collection.to_code()
    ...
    def update_plot(self):
        ...
        # draw the lines
        for line in lines:
            x_coords = [point.provisional_x for point in line.points]
            y_coords = [point.provisional_y for point in line.points]

            # Create a new PlotDataItem for each line and add it to the plot
            line_item = pg.PlotDataItem(
                x_coords, y_coords, pen=pg.mkPen(width=2, color="g")
            )
            self.plot.addItem(line_item)

            # TODO: MAKE SURE LINES GET REMOVED WHEN CLEARING THEM FROM THE CODE. NOW THEY ARE ADDED TO THE CODE AND THEY ARE NEVER CLEARED.
